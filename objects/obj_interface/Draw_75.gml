...
draw_rectangle(xx, 28, xx + (w * 32) - 1, 28 + (h * 32) - 1, false);
			
draw_set_colour(c_white);
draw_rectangle(xx, 28, xx + (w * 32) - 1, 28 + (h * 32) - 1, true);
...
for(var _y = 0; _y < h && i < tile_count; _y ++)
{
	...
	{
		draw_sprite_stretched(tile_icons[i], 0, xx + (_x * 32), 28 + (_y * 32), 32, 32);
		i ++;
	}
}
...
draw_rectangle(xx + cellS.x + 1, 28 + cellS.y + 1, 768 + cellS.x + 31 - 1, 28 + cellS.y + 31 - 1, true);
...
if tile_hovered != undefined
{
	...
	draw_set_colour(c_white);
	draw_rectangle(xx + cellS.x, 28 + cellS.y, 768 + cellS.x + 31, 28 + cellS.y + 31, false);
