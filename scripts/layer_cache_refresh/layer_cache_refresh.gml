function layer_cache_refresh(this_chunk, this_layer)
{
	...
	for ( var _x = 0; _x < 32; _x ++ ) 
	{
		...
		{
			var this_tile = global.chunk[? this_chunk].layers[| this_layer].tiles[# _x, _y];
			...
			switch this_type.model
				{
					...
					case "plane":
						...
						if this_type.type == "grass"
						{
							...
							{
								if (_x + _xx > 31) || global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y].type == undefined || obj_tiles.list[| global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y].type].type != "grass" || ds_list_find_index(skiplist, string(_x + _xx) + "," + string(_y)) != -1 || (_x + _xx) mod 4 == 0 || global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y].z != global.chunk[? this_chunk].layers[| this_layer].tiles[# _x, _y].z
								{
									row_width = _xx;
									break;
								}
							...
							for ( var _xx = 0; _xx < row_width; _xx ++ )
							{
								...
								{
									if (_y + _yy > 31)  || global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y + _yy].type == undefined || obj_tiles.list[| global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y + _yy].type].type != "grass" || ds_list_find_index(skiplist, string(_x + _xx) + "," + string(_y + _yy)) != -1 || (_y + _yy) mod 4 == 0 || global.chunk[? this_chunk].layers[| this_layer].tiles[# _x + _xx, _y + _yy].z != global.chunk[? this_chunk].layers[| this_layer].tiles[# _x, _y].z
									{
	...
	if tiles_placed == 0
	{
		...
		if global.chunk[? this_chunk].render_cache[this_layer] != undefined
				sprite_delete( global.chunk[? this_chunk].render_cache[this_layer]);
		
		global.chunk[? this_chunk].render_cache[this_layer] = undefined;
		return;
	}
	else 
	{
		...
		global.chunk[? this_chunk].render_cache[this_layer] = sprite_create_from_surface(surf_layercache, 0, 0, 72, 72, false, false, 0, 0);
