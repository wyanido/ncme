function layer_icon_refresh(this_chunk, this_layer = undefined)
{
	with (obj_layers)
	{
		// Draw layer mesh to a surface and save it as the layer icon
		this_layer ??= sel;
		...
		else
		{
		 ...
		 if (is_undefined(cache[? this_chunk])) {
			cache[? this_chunk] = array_create(LAYER_COUNT, undefined);
		 } 
		 else if (!is_undefined(cache[? this_chunk][this_layer])) {
